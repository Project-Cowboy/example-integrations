...
pub async fn run(node_url: &str) {
    ...
    let json_str =
        fs::read_to_string("tlsn_tiktok_viewer_proof.json").expect("Failed to read file");
    ...
    // extract the receipt, an object containing the proof and public outputs
    let receipt = prove_info.receipt;

    receipt.verify(COWBOY_EXAMPLE_APPS_ID).unwrap();

    let proof_output: u8 = receipt
        .journal
        .decode()
    ...
    println!(
        ...
        AccountType::try_from(proof_output)
            .expect("Should not have created an account type identifier that is out of range.")
    );
}
