...
pub async fn upload() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Url identifier of integration: api.x.com/1.1/account/settings.json
    let selector_app_host = b"api.x.com".to_vec();
    let selector_app_uri = b"/1.1/account/settings.json".to_vec();
