...
pub async fn run(base_url: &str) {
    ...
    let prove_info = prover
        .prove(env, COWBOY_EXAMPLE_APPS_ELF)
        .unwrap();
    ...
    receipt
        .verify(COWBOY_EXAMPLE_APPS_ID)
        .unwrap();

    let proof_output: [u8; 32] = receipt.journal.decode().expect("Journal should contain value");

    println!(
        "All verified. Committed(public) output is hashed usermane: \"{:?}\" ",
        proof_output
    );
}
